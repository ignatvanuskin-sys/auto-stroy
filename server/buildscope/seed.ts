...
const leadSeeds = [
  [
    ...
    "Qualified",
    92,
    "Клиент готов обсуждать проект; участок оформлен, ищет подрядчика с опытом монолитного строительства.",
  ],
  [
    "Алексей Морозов",
    "+7 777 614 02 18",
    ...
    "Negotiation",
    96,
    "Есть участок в Медеуском районе и готовый эскиз. Критичны сроки и прозрачная смета.",
  ],
  [
    "Айша Тулеубаева",
    "+7 705 327 19 60",
  ...
  for (let index = 0; index < leadSeeds.length; index++) {
    ...
    await db.insert(leadActivities).values([
      ...
      },
      {
        ...
        type: "status_changed",
        payload: { to: status },
        createdAt: daysAgo(Math.max(0, 1 + index)),
      },
    ]);
  ...
  await db.insert(tasks).values([
    {
      companyId,
      leadId: leadIds[0]!,
      title: "Подтвердить время звонка с Данияром",
      dueAt: new Date(now + 2 * 3_600_000),
    },
    {
      companyId,
      leadId: leadIds[1]!,
      title: "Подготовить уточнение по эскизу",
      dueAt: new Date(now + 24 * 3_600_000),
    },
    {
      companyId,
      leadId: leadIds[7]!,
      title: "Согласовать доп. опции навеса",
