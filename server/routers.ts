...
async function resolveEstimate(
  ...
  const currentRate = await activeRate(
    companyId,
    input.region,
    input.material,
    input.finishTier
  ...
  return estimateProject({
    ...input,
    baseRatePerM2: currentRate?.baseRatePerM2,
  });
}
...
export const appRouter = router({
  ...
  calculator: router({
    ...
    submitLead: leadRateLimit.input(leadSchema).mutation(async ({ input }) => {
      ...
      await db.insert(leadActivities).values([
        {
          ...
          payload: {
            source: "Website",
            calculation: formatEstimateRange(estimate),
          },
        },
        {
          companyId,
      ...
      return {
        ...
        estimate: {
          ...estimate,
          formattedRange: formatEstimateRange(estimate),
        },
        score: scoring,
        aiSummary: ai.summary,
        needsManualReview: ai.needsManualReview || scoring.budgetMismatch,
  ...
  crm: router({
    overview: publicProcedure.query(async () => {
      ...
      const totalPipeline = active.reduce(
        (sum, lead) => sum + (lead.score > 0 ? lead.score * 515_000 : 0),
        0
      );
      const byStatus = [
        "New",
      ...
      const bySource = [
        ...
        source,
        count: list.filter(lead => lead.source === source).length,
      }));
      ...
      return {
        ...
        bySource,
        responseTime: "18 мин",
        proposalCount: list.filter(lead =>
          ["Proposal Sent", "Negotiation", "Won"].includes(lead.status)
        ).length,
        wonCount: list.filter(lead => lead.status === "Won").length,
      };
    }),
      ...
      .input(
        z
          .object({
            ...
            scoreBand: z.enum(["cold", "warm", "hot", "very_hot"]).optional(),
          })
          .optional()
      )
      .query(async ({ input }) => {
        ...
        if (input?.status)
          list = list.filter(lead => lead.status === input.status);
        if (input?.scoreBand)
          list = list.filter(lead => lead.scoreBand === input.scoreBand);
        return list;
      }),
    getLead: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [activity, estimateList, taskList, followupList, proposalList] =
          await Promise.all([
            db
              .select()
              .from(leadActivities)
            ...
            db
              ...
              .from(tasks)
              .where(
                and(eq(tasks.companyId, companyId), eq(tasks.leadId, lead.id))
              )
    ...
    generateProposal: crmMutationProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const estimate = {
          lowAmount: latest.lowAmount,
          highAmount: latest.highAmount,
          baseRatePerM2: 0,
          factors: [],
        };
        const proposalNumber = `BS-${lead.id.toString().padStart(4, "0")}-${new Date().getFullYear()}`;
        const pdfBase64 = buildProposalPdf({
          proposalNumber,
        ...
        return {
          ...
          pdfBase64,
          preview: {
            title: "Предварительное коммерческое предложение",
            range: formatEstimateRange(estimate),
            disclaimer:
              "Это предварительная оценка. Точный расчёт менеджер подготовит после уточнения деталей проекта.",
          },
        };
      }),
    notifications: publicProcedure.query(async () =>
      listCompanyNotifications(await getDemoCompanyId())
    ),
    ...
    updateRate: crmMutationProcedure
      .input(
        z.object({
          ...
          baseRatePerM2: z.number().int().min(100000).max(800000),
        })
      )
      .mutation(async ({ input }) => {
        const db = await getDb();
