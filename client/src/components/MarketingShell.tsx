    ...
    <div className={dark ? "bg-[#202a28] text-[#eff1e9] min-h-screen" : "bg-[#f5f3ed] text-[#18201f] min-h-screen"}>
      <header className="relative z-30 border-b border-current/15">
        <div className="mx-auto flex max-w-[1360px] items-center justify-between px-5 py-5 lg:px-9">
          ...
          <button aria-label="Открыть меню" className="grid h-10 w-10 place-items-center border border-current/20 lg:hidden" onClick={() => setOpen(!open)}>{open ? <X size={18} /> : <Menu size={19} />}</button>
        </div>
        {open && <div className="absolute left-0 top-full w-full border-b border-current/15 bg-inherit p-5 lg:hidden"><div className="mx-auto grid max-w-[1360px] gap-1">{nav.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="border-b border-current/10 py-4 text-sm font-bold">{label}</Link>)}<Link href="/crm/dashboard" onClick={() => setOpen(false)} className="border-b border-current/10 py-4 text-sm font-bold">CRM / demo</Link><Link href="/calculator" onClick={() => setOpen(false)} className="btn-primary mt-4">Рассчитать проект <ArrowUpRight size={15}/></Link></div></div>}
      </header>
      {children}
